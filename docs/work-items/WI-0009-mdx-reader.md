...
- 2026-08-09: implemented in src/formats/mdx by a subagent (fa7de71, "Add MDX model reader"): typed data for all 20 occurring chunks, one generic keyframe-track parser and one generic node parser, every count validated before allocating. The layout was pre-validated with an independent Python walker over the whole corpus, so the sweep converged on the first run.
- 2026-08-09: spec deviations established against the corpus, commented at their parse sites: geoset per-sequence extents are 28 bytes (bounds radius + min + max), not Magos's 24 (roughly half the corpus misparses at 24; e.g. Footman.mdx, 13 extents/geoset); PRE2 carries a KP2L latitude track (31 files, e.g. MonsoonBoltTarget.mdx) and cameras carry KCRL rotation tracks (124 files) the spec omits; light attenuation and camera fov/near/far are floats despite being typed DWORD. Corpus facts locked in: UVAS count = number of following UVBS sets (1 to 3), CLID types only box (276 files) and sphere (619), ATCH fixed part is path[260]+id, face type always 4 (triangles).
- 2026-08-09: sweep on this machine: War3.mpq parsed 2049, War3x.mpq parsed 1313, 3362/3362 with zero failures. Corpus spot-checks (Footman, CentaurTent1) assert values derived from an independent Python reader. A stress probe ran ~353k truncation parses and 250k single-byte corruptions across models covering every chunk parser: no crashes, leaks, or repeat frees under a tracking allocator.
- 2026-08-09: independent review (second subagent, uninvolved in the implementation): approved, no HIGH or MEDIUM findings. It independently re-decoded Grunt, MonsoonBoltTarget, HumanTower, and LightningBoltMissile (none used by the committed tests) with its own Python reader and matched the parser bit-exactly, confirmed the 28-byte extent and KP2L/KCRL deviations structurally, re-ran the sweep (same counts), and ran its own 136k-parse stress probe (clean). Four LOW findings, all fixed in e3cc64b ("Fix MDX review findings"): validation arithmetic widened to i64 so a 32-bit int target (wasm) cannot wrap a count product into a passing check; duplicate typed-chunk detection moved to a seen-tag set, closing the empty-first-chunk loophole; six new tests pin the previously uncovered error paths; this log entry closes the bookkeeping gap. Sweep re-run after the fixes: 3362/3362. Status to done.
